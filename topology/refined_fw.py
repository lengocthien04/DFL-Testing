...
def symmetrize_and_row_normalize(W: np.ndarray) -> np.ndarray:
    """
    Enforces:
    - symmetry
    - non-negativity
    ...
    This corresponds to the feasibility assumptions used
    when applying W as a mixing matrix in decentralized SGD.
    """
    W = 0.5 * (W + W.T)              # enforce symmetry
    W = np.maximum(W, 0.0)           # enforce non-negativity
...
def stl_fw(Pi: np.ndarray, lam: float, iters: int) -> np.ndarray:
    ...
    # Initialization (identity is an extreme point of permutation hull)
    ...
    # Enforce feasibility AFTER FW optimization (paper-compatible)
    W = symmetrize_and_row_normalize(W)
    return W
...
def build(
    ...
    # Adjacency extracted for communication graph
